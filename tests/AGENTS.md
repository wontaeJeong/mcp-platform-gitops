...
| `render-production.sh` | Fixed platform roots, MCP server catalog roots, ApplicationSet shape, security, TLS, sync-wave, and README invariants |
...
- ApplicationSet assertions must keep the `apps/mcp-servers/*` generator, `<name>-mcp-server` naming, foreground finalizer, prune behavior, and stale path rejection intact.
...
- Do not reintroduce the old flat mock path or an explicit mock Application apply flow in docs or tests.
